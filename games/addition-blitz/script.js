(function(){
  ...
  var missCounts = {};      // key "a-b" -> times missed this session
  ...
  // Picks an addition pair. Most of the time generates a fresh random pair
  // within the chosen range; sometimes replays a fact missed earlier this
  // session. Avoids repeating the immediately previous question.
  function pickPair(){
    ...
    if (missKeysList.length > 0 && Math.random() < REPLAY_MISS_PROBABILITY){
      ...
      do{
        pair = [randInt(0, max), randInt(0, max)];
        attempts++;
      } while (recentKeys.indexOf(sortedKey(pair)) !== -1 && attempts < 6);
    }

    return pair;
  }

  function sortedKey(pair){
  ...
  function renderQuestion(){
    ...
    var a = pair[0], b = pair[1];
    currentAnswer = a + b;
    currentKey = sortedKey(pair);

    recentKeys.push(currentKey);
    if (recentKeys.length > 2) recentKeys.shift();

    questionText.innerHTML = a + ' <span class="op">+</span> ' + b;
